...
class accGrePartIp {
    ...
    private static $tableName = 'ops_part_pool';
...
//        $conn = mysqli_connect($sqlConfig['host'],$sqlConfig['user'],$sqlConfig['password']);
        ...
        $insert_sql = "insert into $table_name (region,set_name,campus,network_ip,start_ip,end_ip,broadcast_ip,create_time) values ('$region','$set','$campus','$network_ip','$start_ip','$end_ip','$broadcast_ip',$create_time);";
        mysqli_query($conn,"SET NAMES 'UTF8'");
        $result = mysqli_query($conn,$insert_sql);
        //写入日志并判断是否插入数据库成功
        if ($result) {
            echo 'add success :'.$insert_sql."\n";
            file_put_contents(ROOT_PATH."/ops_info_success.log",$insert_sql."\n",FILE_APPEND);
        }else {
            echo 'add failed :'.$insert_sql."\n";
            file_put_contents(ROOT_PATH."/ops_info_failed.log",$insert_sql."\n",FILE_APPEND);
        }
        mysqli_close($conn);
    }
