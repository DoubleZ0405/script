...
class accPartTpcIp {
    ...
    private static $tableName = 'tpc_part_pool';
        ...
        try {
            ...
            for($row=1; $row<=$highestRow; $row++) {//从第一行开始读取数据
                ...
                $dbParam = array(
                    ...
                    'campus' => $rowData[0][2],
                    'ops_id' => $rowData[0][3],
                    'network_ip' => $rowData[0][5],
                    'start_ip' => $rowData[0][6],
                    'end_ip' => $rowData[0][7],
                    'broadcast_ip' => $rowData[0][8],
                );
...
//         }
        ...
        $insert_sql = "insert into $table_name (region,set_name,campus,ops_id,network_ip,start_ip,end_ip,broadcast_ip,create_time) values ('$region','$set','$campus','$ops_id','$network_ip','$start_ip','$end_ip','$broadcast_ip',$create_time);";
        // var_dump($insert_sql);
        var_dump(0);die();
        ...
        if ($result) {
            ...
            file_put_contents(ROOT_PATH."/tpc_info_failed.log",$insert_sql."\n",FILE_APPEND);
        }
        // mysqli_close($conn);
    }
